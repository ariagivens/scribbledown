...
export async function create_epub(
    ...
    const package_opf = package_opf_template({ title, uid, chapters });
    ...
    for (const chapter of chapters) {
        const chapter_xhtml = chapter_xhtml_template(chapter);
        oebps.file(chapter.identifier + ".xhtml", chapter_xhtml);
    }
